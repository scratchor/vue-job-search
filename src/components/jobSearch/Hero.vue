<template>
  <main class="flex flex-col h-screen">
    <section class="flex flex-col pt-10 pb-20">
      <div class="grid grid-cols-12">
        <div class="col-start-1 col-span-1"></div>
        <div class="col-start-2 col-span-5">
          <headline />
          <job-search-form />
        </div>
        <div class="col-start-7 col-span-5 self-center justify-self-center">
          <img
            src="https://upload.wikimedia.org/wikipedia/commons/thumb/9/95/Vue.js_Logo_2.svg/2367px-Vue.js_Logo_2.svg.png"
            alt="image"
            class="w-80 h-80 object-contain"
          />
        </div>
        <div class="col-start-12 col-span-1"></div>
      </div>
    </section>

    <spotlight class="flex flex-row justify-center pb-16">
      <template #default="{ img, title, description }">
        <!--{ spotlight: {} }-->
        <router-link
          to="/jobs/result"
          class="flex flex-col w-72 mx-5 border rounded-lg bg-bgray-2 h-96"
        >
          <img class="object-contain" :src="img" alt="spotlights image" />

          <div class="h-48 px-6 py-4 mt-3">
            <h3 class="text-lg font-medium">
              {{ title }}
            </h3>

            <p class="mt-3 text-sm">
              {{ description }}
            </p>
          </div>

          <router-link to="/jobs/result" class="px-6 pb-4 text-sm text-bb-1"
            >See jobs</router-link
          >
        </router-link>
      </template>
    </spotlight>
  </main>
</template>

<script>
import Headline from "@/components/jobSearch/Headline";
import JobSearchForm from "@/components/jobSearch/JobSearchForm";
import Spotlight from "@/components/jobSearch/Spotlight";

export default {
  name: "Hero",
  components: {
    Headline,
    Spotlight,
    JobSearchForm,
  },
  beforeCreate() {
    console.log("Hero beforeCreate");
  },
  created() {
    console.log("Hero created");
  },
  beforeMount() {
    console.log("Hero beforeMount");
  },
  mounted() {
    console.log("Hero mounted");
  },
};
</script>

<style scoped></style>
